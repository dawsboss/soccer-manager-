...
- Every suite also runs on its own when you want the detail of one area:
  ...
  - `node test/roles.js`, `node test/visibility.js` — roles derived from where a uid appears; which teams each account sees and may change.
...
Pinned today: `wireBase()` replaces local state wholesale on the connect-time read (`app.js:439`), against the invariant below. `node test/run.js` lists every gap at the end.
