...
- Every suite also runs on its own when you want the detail of one area:
  ...
  - `node test/ai.js` — the Ask an AI prompt builder. Builds every prompt and fails if any roster name or player note is in it, checks that shared or missing shirt numbers never merge two players, and that trackers, parents and non-admins are refused.
